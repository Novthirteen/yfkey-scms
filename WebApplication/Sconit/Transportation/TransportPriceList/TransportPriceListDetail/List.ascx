...
<fieldset>
    <div class="GridView">
        <cc1:GridView ID="GV_List" runat="server" AutoGenerateColumns="False" DataKeyNames="Id"
            ...
            <Columns>
                <asp:TemplateField HeaderText="${Transportation.TransportPriceListDetail.ShipFrom}" SortExpression="ShipFrom">
                    <ItemTemplate>
                        <%# DataBinder.Eval(Container.DataItem, "ShipFrom.FullAddress")%>
                    </ItemTemplate>
                </asp:TemplateField>      
                <asp:TemplateField HeaderText="${Transportation.TransportPriceListDetail.ShipTo}" SortExpression="ShipTo">
                    <ItemTemplate>
                        <%# DataBinder.Eval(Container.DataItem, "ShipTo.FullAddress")%>
                    </ItemTemplate>
                </asp:TemplateField>  
                <asp:TemplateField HeaderText="${Transportation.TransportPriceListDetail.PricingMethod}" SortExpression="PricingMethod">
                    <ItemTemplate>
                ...
                <asp:TemplateField HeaderText="${Transportation.TransportPriceListDetail.VehicleType}" SortExpression="VehicleType">
                    <ItemTemplate>
                        <cc1:CodeMstrLabel ID="lblVehicleType" runat="server" Code="VehicleType" Value='<%# Bind("VehicleType") %>' />
                         <asp:Label ID="lbVehicleType" runat="server" Text='<%# Eval("VehicleType")%>'
                            CssClass="hidden" />
                    </ItemTemplate>
                </asp:TemplateField> 
