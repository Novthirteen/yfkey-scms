<%@ Control Language="C#" AutoEventWireup="true" CodeFile="List.ascx.cs" Inherits="Transportation_TransportPriceList_Operation_List" %>
...
<fieldset>
    <div class="GridView">
        <cc1:GridView ID="GV_List" runat="server" AutoGenerateColumns="False" DataKeyNames="Id"
            ...
            <Columns>
                <asp:TemplateField HeaderText="${MasterData.Item.Code}" SortExpression="Item">
                    <ItemTemplate>
                        <%# DataBinder.Eval(Container.DataItem, "Item.Code")%>
                    </ItemTemplate>
                </asp:TemplateField>    
                <asp:TemplateField HeaderText="${Transportation.TransportPriceListDetail.BillingMethod}" SortExpression="BillingMethod">
                    <ItemTemplate>
                        <cc1:CodeMstrLabel ID="lblBillingMethod" runat="server" Code="BillingMethod" Value='<%# Bind("BillingMethod") %>' />
                         <asp:Label ID="lbBillingMethod" runat="server" Text='<%# Eval("BillingMethod")%>'
                            CssClass="hidden" />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="${Transportation.TransportPriceListDetail.UnitPrice}" SortExpression="UnitPrice">
                    <ItemTemplate>
