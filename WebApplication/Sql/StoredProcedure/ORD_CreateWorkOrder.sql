...
BEGIN
	...
	create table #tempOrderLocTrans
	(
	...
	begin try
		...
		--分解Bom
		insert into #tempBomDetail exec ORD_GetFlatBomDetail @Bom, @DateTimeNow
		...
		--没有工序号赋值默认工序
		update #tempBomDetail set Op = @DefaultOp where Op is null
		select @MinOp = MIN(OP), @MaxOp = MAX(Op) from #tempBomDetail

		--记录工单产出
		insert into #tempOrderLocTrans 
		(
		Item,
		ItemDesc,
		Uom,
		...
		from #tempOrderLocTrans as olt inner join RoutingDet as rd on rd.Routing = @Routing and olt.Op = rd.Op
		...
		--计算单位换算（Bom单位转为基本单位）
		update #tempOrderLocTrans set UnitQty = 1 where Uom = BaseUom and UnitQty is null
		update olt set UnitQty = c.BaseQty / c.AltQty
		from #tempOrderLocTrans as olt inner join UomConv as c on olt.Item = c.Item and olt.Uom = c.AltUom and olt.BaseUom = c.BaseUom
		where olt.UnitQty is null
		update olt set UnitQty =  c.AltQty / c.BaseQty
		from #tempOrderLocTrans as olt inner join UomConv as c on olt.Item = c.Item and olt.Uom = c.BaseUom and olt.BaseUom = c.AltUom
		where olt.UnitQty is null
		update olt set UnitQty = c.BaseQty / c.AltQty
		from #tempOrderLocTrans as olt inner join UomConv as c on olt.Uom = c.AltUom and olt.BaseUom = c.BaseUom 
		where olt.UnitQty is null and c.Item is null
		update olt set UnitQty =  c.AltQty / c.BaseQty
		from #tempOrderLocTrans as olt inner join UomConv as c on olt.Uom = c.BaseUom  and olt.BaseUom = c.AltUom 
		where olt.UnitQty is null and c.Item is null
		
		if exists(select top 1 1 from #tempOrderLocTrans where UnitQty is null)
		begin
			select top 1 @ErrorMsg = N'没有维护原材料[' + Item + N']从计量单位[' + Uom + N']转换为计量单位[' + BaseUom + N']的换算关系。' from #tempOrderLocTrans where UnitQty is null
			RAISERROR(@ErrorMsg, 16, 1)
		end

		--Bom总用量、单位用量、箱数转为基本单位的用量
		update #tempOrderLocTrans set OrderQty = OrderQty * UnitQty, BomUnitQty = BomUnitQty * UnitQty, Cartons = ROUND(OrderQty / UC, 0)
